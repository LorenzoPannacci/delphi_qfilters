...
async def run(run_cfg: RunConfig):
    ...
    # non_redundant_hookpoints() returns list of hookpoints not already in the cache
    nrh = assert_type(
        dict,
        non_redundant_hookpoints(
    ...
    # if there all hookpoints have been already cached the step is skipped
    if nrh:
        populate_cache(
            run_cfg,
            model,
    ...
    # what are neighbours?
    if run_cfg.constructor_cfg.non_activating_source == "neighbours": # if we want neighbours
        ...
        # if not yet computed create neighbours
        if nrh:
            create_neighbours(
                run_cfg,
                latents_path,
    ...
    # if cache has already been processed skip
    nrh = assert_type(
        list,
        non_redundant_hookpoints(
