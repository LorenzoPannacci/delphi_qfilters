            ...
            for batch_number, batch in enumerate(token_batches):
                ...
                with torch.no_grad():
                    ...
                    for layer in self.layers:
                        ...
                        attentions = outputs.attentions[layer]  # (layers, batch, heads, seq_len, seq_len) -> (batch, heads, seq_len, seq_len)

                        avg_attention = torch.sum(attentions, dim=2) / (attentions != 0).sum(dim=2) # -> (batch, heads, seq_len) | torch.Size([32, 32, 256])
                        avg_attention = avg_attention.permute(0, 2, 1).contiguous() # -> (batch, seq_len, heads) | torch.Size([32, 256, 32])

                        activation_score = avg_attention

                        self.cache.add(activation_score, batch, batch_number, layer)

                        # count firings
                        firing_counts = (activation_score > 0).sum((0, 1))

                        if self.width is None:
                            self.width = activation_score.shape[2]
