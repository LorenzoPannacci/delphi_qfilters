...
from delphi.qfilters.src.utils import QFilters
...
def get_nonzeros_batch(
    ...
    for i in range(0, latents.shape[0], max_batch_size):
        ...
        batch_locations = torch.nonzero(batch.abs() > 0)
        batch_activations = batch[batch.abs() > 0]
...
class InMemoryCache:
    ...
    def add(
        ...
        """
        # self.cache.add(avg_attention, batch, batch_number, layer)
    ...
    def save(self):
        ...
        paths = list(self.latent_locations_batches.keys())
        ...
        for key in list(self.latent_locations_batches.keys()):
            del self.latent_locations_batches[key]
        self.latent_locations_batches.clear()
        self.latent_locations_batches = None  
        gc.collect()

        # If using CUDA, free cached GPU memory
        ...
        for key in list(self.latent_activations_batches.keys()):
            del self.latent_activations_batches[key]
        self.latent_activations_batches.clear()
        self.latent_activations_batches = None  
        gc.collect()
        ...
        for key in list(self.tokens_batches.keys()):
            del self.tokens_batches[key]
        self.tokens_batches.clear()
        self.tokens_batches = None  
        gc.collect()
    ...
    def get_nonzeros(self, latents: latent_tensor_type, module_path: str) -> tuple[
        ...
        if size > torch.iinfo(torch.int32).max:
            ...
            nonzero_latent_locations = torch.nonzero(latents.abs() > 0) # modified
            nonzero_latent_activations = latents[latents.abs() > 0] # modified
...
class LatentCache:
    ...
    def __init__(
        ...
        hookpoint_to_sparse_encode: dict[str, Callable], # dictionary made of name and SAE callable
        batch_size: int,
        transcode: bool = False,
        filters: dict[str, Float[Tensor, "indices"]] | None = None, # what are filters?
        ...
        self.layers = [int(elem) for elem in layers]
    ...
    def run(self, n_tokens: int, tokens: token_tensor_type):
        ...
        q_filters = QFilters.from_pretrained(f"nthngdy/{self.model_name.split('/')[-1]}_qfilt").to(device)
        ...
        with tqdm(total=total_batches, desc="Caching latents") as pbar:
            for batch_number, batch in enumerate(token_batches):
                ...
                with torch.no_grad():
                        ...
                        # this generates the model activations
                        model_out = self.model(batch.to(self.model.device))

                        kv_cache = model_out.past_key_values
                        key_layers = kv_cache.key_cache

                        for layer in self.layers:

                            # get keys of layer
                            # k: batch_size x num_heads x seq_len x head_dim
                            keys = key_layers[layer]

                            # get q filter of layer
                            # q: num_heads x head_dim
                            q_filter = q_filters.q_filters[layer].to(keys.dtype)

                            # get Q filters scores
                            # following is equivalent to a scalar product as in the paper: <u_h, k_i>
                            qfilter_scores = torch.einsum("hd,bhsd->bsh", q_filter, keys)

                            # the minus sign to follow what is written on the paper
                            qfilter_scores = -qfilter_scores

                            self.cache.add(qfilter_scores, batch, batch_number, layer)

                            # count firings
                            firing_counts = (qfilter_scores.abs() > 0).sum((0, 1))

                            if self.width is None:
                                self.width = qfilter_scores.shape[2]

                            if str(layer) not in self.hookpoint_firing_counts:
                                self.hookpoint_firing_counts[str(layer)] = (firing_counts.cpu())
                            else:
                                self.hookpoint_firing_counts[str(layer)] += firing_counts.cpu()
        ...
        # at the end of the processing log and save data
    ...
    def save_splits(self, n_splits: int, save_dir: Path, save_tokens: bool = True):
        ...
        for module_path in self.cache.latent_locations.keys():
            ...
            for start, end in split_indices:
                ...
                module_dir = save_dir / str(module_path)
    ...
    def save_config(self, save_dir: Path, cfg: CacheConfig, model_name: str):
        ...
        """
        for module_path in self.cache.latent_locations.keys():
            config_file = save_dir / str(module_path) / "config.json"
