            ...
            for batch_number, batch in enumerate(token_batches):
                ...
                with torch.no_grad():
                        ...
                        for layer in self.layers:
                            ...
                            keys = key_layers[layer] # torch.Size([32, 8, 256, 64])

                            l2norm_scores = torch.norm(keys, dim=-1) # torch.Size([32, 8, 256])
                            l2norm_scores = l2norm_scores.permute(0, 2, 1)

                            self.cache.add(l2norm_scores, batch, batch_number, layer)

                            # count firings
                            firing_counts = (l2norm_scores.abs() > 0).sum((0, 1))

                            if self.width is None:
                                self.width = l2norm_scores.shape[2]
